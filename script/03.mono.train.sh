...
if [ ! -f $dir/00.mdl ] || [ ! -f $dir/tree ]; then
  ...
  gmm-init-mono --binary=false $shared_phones_opt \
    "--train-feats=ark:subset-feats --n=10 \"ark:$feat\" ark:- |" train/topo $dim \
    $dir/00.mdl $dir/tree 2> $log || exit 1
else
...
if [ ! -f $dir/log/done.graph ] || [ ! -f $dir/train.graph ]; then
  ...
  compile-train-graphs $dir/tree $dir/00.mdl train/L.fst ark:train/train.int \
    ark:$dir/train.graph 2> $log || exit 1
...
if [ ! -f $dir/log/done.00.ali ] || [ ! -f $dir/00.ali ]; then
  ...
  echo "        log -> $log"
  align-equal-compiled ark:$dir/train.graph "ark:$feat" \
    ark:$dir/00.ali 2> $log || exit 1
...
if [ ! -f $dir/00.acc ]; then
  ...
  echo "        log -> $log"
  gmm-acc-stats-ali --binary=false $dir/00.mdl "ark:$feat" \
    ark:$dir/train.ali $dir/00.acc 2> $log || exit 1
else
...
if [ ! -f $dir/01.mdl ]; then
  ...
  gmm-est --binary=false --min-gaussian-occupancy=3 --mix-up=$numgauss \
    $dir/00.mdl $dir/00.acc $dir/01.mdl 2> $log || exit 1
else
...
while [ $iter -le $numiters ] ; do
  ...
  echo "Iteration $x :"
    if echo $realign_iters | grep -w $iter >/dev/null ; then #align
      ...
      gmm-align-compiled $scale_opts --beam=$beam --retry-beam=$[$beam*4] \
        $dir/$x.mdl ark:$dir/train.graph "ark:$feat" \
        ark:$dir/$x.ali 2> $log || exit 1
  ...
  echo "       log    ->  $log"
  gmm-acc-stats-ali --binary=false $dir/$x.mdl "ark:$feat" \
    ark:$dir/train.ali $dir/$x.acc 2> $log || exit 1

  echo "      updating gmm parameters and splitting to [ $numgauss ] gaussians"
  log=$dir/log/update.$x.log
  echo "       output ->  $dir/$x.mdl"
  echo "       log    ->  $log"
